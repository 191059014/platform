package com.hb.platform.unic.common.security;

import com.hb.platform.unic.common.util.HexByteArrUtils;
...
public enum SymmetricEncrypt {
    ...
    DES("DES", "DES/ECB/PKCS5Padding", "DES对称加密") {
        ...
        public String encode(String message, String secretKey) {
            try {
                return HexByteArrUtils.byteArr2HexStr(
                    jdkDes(message.getBytes(), secretKey, getAlgorithm(), getFillStyle(), Cipher.ENCRYPT_MODE));
            } catch (Exception e) {
                LOGGER.error("{}加密异常, algorithm={}, message={}, secretKey={}", getAlgorithm(), message, secretKey);
                throw new RuntimeException(e);
            }
        }

        @Override
        public String decode(String message, String secretKey) {
            try {
                return new String(jdkDes(HexByteArrUtils.hexStr2byteArr(message), secretKey, getAlgorithm(),
                    getFillStyle(), Cipher.DECRYPT_MODE));
            } catch (Exception e) {
                LOGGER.error("{}解密异常, algorithm={}, message={}, secretKey={}", getAlgorithm(), message, secretKey);
                throw new RuntimeException(e);
            }
        }
    ...
    AES("AES", "AES/ECB/PKCS5Padding", "AES对称加密") {
        ...
        public String encode(String message, String secretKey) {
            try {
                return HexByteArrUtils.byteArr2HexStr(
                    jdkAes(message.getBytes(), secretKey, getAlgorithm(), getFillStyle(), Cipher.ENCRYPT_MODE));
            } catch (Exception e) {
                LOGGER.error("{}加密异常, algorithm={}, message={}, secretKey={}", getAlgorithm(), message, secretKey);
                throw new RuntimeException(e);
            }
        ...
        public String decode(String message, String secretKey) {
            try {
                return new String(jdkAes(HexByteArrUtils.hexStr2byteArr(message), secretKey, getAlgorithm(),
                    getFillStyle(), Cipher.DECRYPT_MODE));
            } catch (Exception e) {
                LOGGER.error("解密异常, algorithm={}, message={}, secretKey={}", getAlgorithm(), message, secretKey);
                throw new RuntimeException(e);
     ...
     * @return 加/解密后的字符串
     */
    protected byte[] jdkAes(byte[] bytes, String secretKey, String algorithm, String fillStyle, int mode)
        throws Exception {
     ...
     * @return byte[]
     */
    protected byte[] jdkDes(byte[] bytes, String secretKey, String algorithm, String fillStyle, int mode)
        throws Exception {
    ...
    public static String generateKey(String algorithm, int keySize) {
        try {
            ...
            LOGGER.error("生成密匙异常, algorithm={}, keySize={}", algorithm, keySize);
            throw new RuntimeException(e);
        }
    }
