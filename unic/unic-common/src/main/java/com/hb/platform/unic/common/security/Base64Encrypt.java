...
public enum Base64Encrypt {
    ...
    JDK {
        ...
        public String encode(String message, String charsetName) {
            try {
                ...
                if (LOGGER.isErrorEnabled()) {
                    LOGGER.error("JDK的Base64加密异常, message={}, charsetName={}", message, charsetName);
                }
        ...
        @Override
        public String decode(String message, String charsetName) {
            try {
                return new String(Base64.getDecoder().decode(message.getBytes(charsetName)), charsetName);
            } catch (Exception e) {
                if (LOGGER.isErrorEnabled()) {
                    LOGGER.error("JDK的Base64解密异常, message={}, charsetName={}", message, charsetName);
                }
                throw new RuntimeException(e);
            }
        }
    ...
    COMMONS_CODEC {
        ...
        public String encode(String message, String charsetName) {
            try {
                return new String(
                    org.apache.tomcat.util.codec.binary.Base64.encodeBase64(message.getBytes(charsetName)),
                    charsetName);
            } catch (Exception e) {
                if (LOGGER.isErrorEnabled()) {
                    LOGGER.error("Commons Codec的Base64加密异常, message={}, charsetName={}", message, charsetName);
                }
                throw new RuntimeException(e);
            }
        }
        ...
        public String decode(String message, String charsetName) {
            try {
                return new String(
                    org.apache.tomcat.util.codec.binary.Base64.decodeBase64(message.getBytes(charsetName)),
                    charsetName);
            } catch (Exception e) {
                if (LOGGER.isErrorEnabled()) {
                    LOGGER.error("Commons Codec的Base64解密异常, message={}, charsetName={}", message, charsetName);
                }
                throw new RuntimeException(e);
            }
        }
